...
class TestGetGraveyardSessions:
    ...
    @patch('src.api.routes.sessions.asyncio')
    @patch('src.api.routes.sessions.get_focus_summary')
    @patch('src.api.routes.sessions.db_get_activity_summaries')
    @patch('src.api.routes.sessions.get_dead_sessions')
    def test_returns_dead_sessions(self, mock_get, mock_summaries, mock_focus, mock_asyncio, client):
        """Test endpoint returns dead sessions."""
        mock_get.return_value = [
            {'sessionId': 'dead-1', 'state': 'dead'},
            {'sessionId': 'dead-2', 'state': 'dead'},
        ]
...
# TestSendMessageToSession and TestWriteToTty removed:
# The write_to_tty function and POST /session/{id}/send endpoint
# were removed in the browser-session-takeover change.
# Session interaction now goes through StreamProcessManager.
